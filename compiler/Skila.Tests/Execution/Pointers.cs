...
namespace Skila.Tests.Execution
{
    ...
    public class Pointers
    {
        ...
        public IInterpreter ExplicitDereferencing()
        {
            ...
            foreach (bool single_mutability in new[] { true, false })
            {
                ...
                VariableDeclaration decl = VariableDeclaration.CreateStatement("x",
                        NameFactory.PointerTypeReference(NameFactory.Int64TypeReference(env.Options.ReassignableTypeMutability())),
                            ExpressionFactory.HeapConstructor(NameFactory.Int64TypeReference(env.Options.ReassignableTypeMutability()),
        ...
        public IInterpreter DereferenceOnAssignment()
        {
            ...
            foreach (bool single_mutability in new[] { true, false })
            {
                ...
                var main_func = root_ns.AddBuilder(FunctionBuilder.Create(
                    ...
                    Block.CreateStatement(new IExpression[] {
                    // p_int *Int = new Int(1)
                    VariableDeclaration.CreateStatement("p_int",NameFactory.PointerTypeReference(NameFactory.Int64TypeReference(env.Options.ReassignableTypeMutability() )),
                        ExpressionFactory.HeapConstructor(NameFactory.Int64TypeReference(env.Options.ReassignableTypeMutability()),
                    ...
                    // z_int Int
                    VariableDeclaration.CreateStatement("z_int",NameFactory.Int64TypeReference(), null,env.Options.ReassignableModifier()),
        ...
        public IInterpreter DirectRefCountings()
        {
            ...
            foreach (bool single_mutability in new[] { true, false })
            {
                var env = Language.Environment.Create(new Options() { DebugThrowOnError = true, AllowInvalidMainResult = true }.SetSingleMutability(single_mutability));
                var root_ns = env.Root;
