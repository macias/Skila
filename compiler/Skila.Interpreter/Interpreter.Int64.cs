...
namespace Skila.Interpreter
{
    ...
    {
        private async Task<ExecValue> executeNativeInt64FunctionAsync(ExecutionContext ctx, FunctionDefinition func, ObjectData thisValue)
        {
            if (func == ctx.Env.Int64ParseStringFunction)
            {
                ...
                if (Int64.TryParse(input_str, out Int64 int_val))
                    int_obj = new Option<ObjectData>(await ObjectData.CreateInstanceAsync(ctx, ctx.Env.Int64Type.InstanceOf, int_val)
            ...
            {
                thisValue.Assign(await ObjectData.CreateInstanceAsync(ctx, thisValue.RunTimeTypeInstance, (Int64)0).ConfigureAwait(false));
            ...
            else if (func == ctx.Env.Int64FromNat8Constructor)
            {
                ObjectData arg_obj = ctx.FunctionArguments.Single();
                var arg_val = arg_obj.NativeNat8;

                thisValue.Assign(await ObjectData.CreateInstanceAsync(ctx, thisValue.RunTimeTypeInstance, (Int64)arg_val).ConfigureAwait(false));
                return ExecValue.CreateReturn(null);
            }
            ...
            {
                ExecValue? result = await numComparisonAsync<Int64>(ctx, func, thisValue).ConfigureAwait(false);
                if (result.HasValue)
                    return result.Value;
                else
