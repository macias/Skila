   ...
   return this.NestedFunctions.Any(it => it.Name.Name == NameFactory.InitConstructorName
       ...
       input.TargetType.AllowSlicedSubstitution) == TypeMatch.Pass);
        ...
        public bool IsValueType(ComputationContext ctx)
        {
            ...
            {
                if (this == ctx.Env.PointerType || this == ctx.Env.ReferenceType)
                    this.isValueType = false;
                else if (this.Modifier.HasEnum || this.Modifier.HasNative || this.Modifier.HasValue)
                    this.isValueType = true;
                else if (this.IsTemplateParameter)
                    this.isValueType = false;
                else if (this.Inheritance.MinimalParentsIncludingObject.Any(it => !it.TargetType.IsValueType(ctx)))
                    this.isValueType = false;
                else if (this.AllNestedFields.Any(it => !it.Evaluation.Aggregate.TargetType.IsValueType(ctx)))
                    this.isValueType = false;
                else
                    this.isValueType = true;
            }
